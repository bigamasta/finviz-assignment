    ...
    <div className="flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded bg-accent-dim flex items-center justify-center text-accent">
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <path d="M7 1L13 4V10L7 13L1 10V4L7 1Z" stroke="currentColor" strokeWidth="1.5" fill="none" />
              <circle cx="7" cy="7" r="1.5" fill="currentColor" />
            </svg>
          </div>
          <span className="font-semibold text-[15px] tracking-tight text-text-1">Taxonomy Explorer</span>
          <span className="text-[10px] uppercase tracking-widest text-text-3 bg-surface-2 px-2 py-0.5 rounded-sm font-mono">ImageNet 2011</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          {isSearching ? (
            <SearchResults key={debouncedSearch} query={debouncedSearch} onSelect={handleSearchSelect} />
          ) : selected ? (
            <NodeDetail node={selected} rootSize={rootSize} />
          ) : (
            <EmptyState />
          )}
        </main>
    ...
    <div className="flex flex-col items-center justify-center h-full gap-3 text-text-3">
      <div className="w-10 h-10 rounded-full bg-surface-2 flex items-center justify-center">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
          ...
          <path d="M3 7l9 4m0 0l9-4m-9 4v10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      </div>
